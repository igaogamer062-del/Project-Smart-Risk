...
(function(){
  ...
  function renderPage(id, user){
    var root = document.getElementById("view-root");
    CSRWorkspace.init({db:()=>db,user:currentUser,perms:()=>userPerms(currentUser()),pages:()=>PAGES,save:()=>saveDB(db),toast:toast});
  ...
  function workspaceBranches(user){
    return {
      ...
      efetivo:[["dashboard","Dashboard do efetivo","chart"],["absence","Faltas","alert"],["vacation","Férias","calendar"],["sanction","Sanções","file"],["control","Controle de efetivo","users"]],
  ...
  function renderWorkspaceNav(user,activeRoute){
    ...
    host.innerHTML='<div class="csr-side-brand"><img src="images/central-smart-risk.png" alt=""><b>Central Smart Risk</b></div><div class="csr-side-scroll">'+items.filter(allowed).map(function(x){return '<div class="csr-nav-group '+(activeRoute===x.route?'active':'')+'"><button class="csr-nav-item" data-route="'+x.route+'">'+svg(x.icon)+'<span>'+x.label+'</span>'+(x.children?'<em>⌄</em>':'')+'</button>'+(x.children?'<div class="csr-nav-children">'+x.children.map(function(c){return '<button data-route="'+x.route+'" data-section="'+c[0]+'">'+svg(({dashboard:'chartLine',control:'user',absence:'absence',vacation:'calendar',sanction:'sanction',overtime:'clock',early:'exit',register:'plus',new:'plus',chat:'chat',config:'config',create:'edit',received:'inbox',history:'history',conversation:'chat',knowledge:'book',manuals:'file',team:'insights',users:'users',profiles:'shield',acessos:'key',abas:'settings'})[c[0]]||'file')+c[1]+'</button>'}).join('')+'</div>':'')+'</div>'}).join('')+'</div><div class="csr-side-foot">Central Smart Risk</div>';
    ...
    if(db.supabaseConnected){
      ...
      try{
        ...
        if(id){
          ...
          addLog("Novo usuário Supabase criado: "+usuario);
          toast("Usuário criado no Supabase.","success");
        }
  ...
  function renderCentralOcorrencias(){
    ...
    function drawTrackingAlerts(){
      ...
      trackingPanel.innerHTML='<div class="ops-head"><div><span class="eyebrow">INTEGRADOR DE RASTREAMENTO</span><h2>Alertas recebidos pela API</h2><p class="hint-text">Eventos normalizados e controlados pelo workflow do backend.</p></div><span class="badge">'+rows.length+' ativo(s)</span></div><div style="overflow:auto"><table class="users-table"><thead><tr><th>Data/hora</th><th>Placa</th><th>Tipo</th><th>Severidade</th><th>Status</th><th>Prazo</th><th>Ação</th></tr></thead><tbody>'+rows.map(function(a){var canRequest=!!a.transporter_id&&["PENDING_TREATMENT","IN_TREATMENT","CLIENT_RESPONDED"].indexOf(a.status)!==-1,canTake=["PENDING_TREATMENT","CLIENT_RESPONDED"].indexOf(a.status)!==-1,canTreat=["IN_TREATMENT","CLIENT_RESPONDED"].indexOf(a.status)!==-1;return '<tr><td>'+escapeHtml(new Date(a.occurred_at).toLocaleString("pt-BR"))+'</td><td><b>'+escapeHtml(a.plate||"—")+'</b></td><td>'+escapeHtml(a.alert_type)+'</td><td><span class="status-pill '+String(a.severity).toLowerCase()+'">'+escapeHtml(a.severity)+'</span></td><td>'+escapeHtml(a.status.replaceAll("_"," "))+'</td><td>'+escapeHtml(new Date(a.deadline_at).toLocaleString("pt-BR"))+'</td><td><div class="row-actions"><button data-tracking-view="'+a.id+'">Detalhes</button>'+(canTake?'<button data-tracking-take="'+a.id+'">Assumir</button>':'')+(canRequest?'<button data-tracking-client="'+a.id+'">Solicitar retorno</button>':'')+(canTreat?'<button data-tracking-treat="'+a.id+'">Concluir</button>':'')+'</div></td></tr>';}).join("")+'</tbody></table></div>';
      trackingPanel.querySelectorAll("[data-tracking-view]").forEach(function(button){button.onclick=function(){var a=rows.find(function(x){return x.id===button.dataset.trackingView;});if(!a)return;document.body.insertAdjacentHTML("beforeend",'<div class="modal-overlay show" id="tracking-detail"><div class="modal-card"><div class="modal-head"><div><small>ALERTA DO INTEGRADOR</small><h2>'+escapeHtml(a.plate||"Alerta")+'</h2></div><button data-track-close>×</button></div><div class="record-detail-grid"><div><span>Tipo</span><b>'+escapeHtml(a.alert_type)+'</b></div><div><span>Status</span><b>'+escapeHtml(a.status.replaceAll("_"," "))+'</b></div><div><span>Motorista</span><b>'+escapeHtml(a.driver_name||"Não informado")+'</b></div><div><span>Origem</span><b>'+escapeHtml(a.provider)+'</b></div><div class="span-all"><span>Descrição</span><p>'+escapeHtml(a.description||"Sem descrição")+'</p></div></div></div></div>');document.querySelector("[data-track-close]").onclick=function(){document.getElementById("tracking-detail").remove();};};});
  ...
  function renderDashboard(){
    ...
    var totalOcorrencias = db.changeLogs.filter(function(c){ return c.tipo === "Alerta gerada"; }).length;
  ...
  /* ---------- anotações ---------- */
  /* ---------- logs do sistema ---------- */
  var CHANGE_TIPOS = ["Usuário criado","Usuário editado","Usuário excluído","Permissões atualizadas","Pronta resposta criada","Pronta resposta editada","Pronta resposta excluída","Alerta gerada","Sinistro criado","Sinistro editado","Sinistro excluído","Perfil atualizado"];
  ...
  function renderEfetivoV4(){
    var root=document.getElementById("efetivo-v2-root"),active="dashboard";
    ...
    function show(which){active=which;if(which==="absence")absence();else if(which==="sanction")sanction();else if(which==="overtime")overtime();else if(which==="control")control();else dashboard();}
