...
(function(){
  ...
  var PAGES = [
    ...
    {id:"config-alertas", label:"Configuração de alertas", perm:"configAbas", icon:"alert"},
    {id:"config-bases", label:"Configurações de bases", perm:"configAbas", icon:"home"},
    {id:"ramais", label:"Ramais das bases", perm:"ramais", icon:"headset"},
    {id:"abrir-chamado", label:"Abrir chamado", perm:"abrirChamado", icon:"chat"},
    {id:"chamados", label:"Chamados recebidos", perm:"chamados", icon:"inbox"},
  ...
  var OP_PERMS = [
    ...
    ["ramais_manage","Ramais — Cadastrar e desativar"]
  ].map(function(x){return {key:x[0],label:x[1]};});

  var DEFAULT_PERMS = {
    Administrador:{menu:true,perfil:true,permissoes:true,usuarios:true,dashboard:true,ocorrencia:true,assistente:true,centralOcorrencias:true,passagem:true,sinistro:true,efetivo:true,checklist:true,ferias:true,configAbas:true,ramais:true,ramais_manage:true,abrirChamado:true,chamados:true,logs:true},
    Coordenador:{menu:true,perfil:true,usuarios:true,dashboard:true,ocorrencia:true,assistente:true,centralOcorrencias:true,passagem:true,sinistro:true,efetivo:true,checklist:true,ferias:true,configAbas:false,ramais:true,abrirChamado:true,chamados:false,logs:true},
    Gerente:{menu:true,perfil:true,permissoes:false,usuarios:true,dashboard:true,ocorrencia:true,assistente:true,centralOcorrencias:true,passagem:true,sinistro:true,efetivo:true,checklist:true,ferias:true,configAbas:false,ramais:true,abrirChamado:true,chamados:false,logs:true},
    Supervisor:{menu:true,perfil:true,permissoes:false,usuarios:true,dashboard:true,ocorrencia:true,assistente:true,centralOcorrencias:true,passagem:true,sinistro:true,efetivo:true,checklist:true,ferias:true,configAbas:false,ramais:true,abrirChamado:true,chamados:false,logs:true},
    Lider:{menu:true,perfil:true,permissoes:false,usuarios:true,dashboard:true,ocorrencia:true,assistente:true,centralOcorrencias:true,passagem:true,sinistro:true,efetivo:true,checklist:true,ferias:true,configAbas:false,ramais:true,abrirChamado:true,chamados:false,logs:true},
    Operador:{menu:true,perfil:true,permissoes:false,usuarios:false,dashboard:true,ocorrencia:true,assistente:true,centralOcorrencias:true,passagem:true,sinistro:true,efetivo:true,checklist:true,ferias:true,configAbas:false,ramais:true,abrirChamado:true,chamados:false,logs:false},
    Cliente:{menu:true,perfil:true,permissoes:false,usuarios:false,dashboard:true,ocorrencia:false,assistente:false,centralOcorrencias:true,passagem:false,sinistro:false,efetivo:false,checklist:false,ferias:false,configAbas:false,ramais:true,abrirChamado:true,chamados:false,logs:false}
  };
  ...
  function seedDB(){
    var db = {
      ...
      transporters: [], operationalBases: [], baseTransporters: [], baseExtensions: [], trackingAlerts: [], supportTickets: []
  ...
  function loadDB(){
    ...
    if(!parsed.transporters) parsed.transporters=[];if(!parsed.operationalBases) parsed.operationalBases=[];if(!parsed.baseTransporters) parsed.baseTransporters=[];if(!parsed.baseExtensions) parsed.baseExtensions=[];if(!parsed.trackingAlerts) parsed.trackingAlerts=[];if(!parsed.supportTickets)parsed.supportTickets=[];
    ...
    parsed.users.forEach(function(u){
      ...
      if(!u.notificationPrefs) u.notificationPrefs = {sinistro:u.notifySinistro !== false, ferias:true, sistema:true, alertas:true, chamados:true};
      ...
      if(u.notificationPrefs.chamados === undefined) u.notificationPrefs.chamados = true;
  ...
  function profileToLocalUser(p){
    return {
      ...
      notificationPrefs:p.notification_preferences||{sinistro:true,ferias:true,sistema:true,alertas:true,chamados:true},
  ...
  function renderPage(id, user){
    ...
    else if(id === "config-alertas") renderAlertConfiguration();
    else if(id === "config-bases") renderBaseConfiguration();
    else if(id === "ramais") renderBaseExtensions();
    else if(id === "abrir-chamado") renderSupportRequest();
    else if(id === "chamados") renderSupportManagement();
  ...
  function renderWorkspaceNav(user,activeRoute){
    ...
    var items=[
      ...
      {route:'ramais',label:'Ramais',icon:'headset'},
      {route:'abrir-chamado',label:'Abrir chamado',icon:'chat'},
      {route:'chamados',label:'Chamados recebidos',icon:'inbox'},
      {route:'ferias',label:'Controle de férias',icon:'calendar'},
      {route:'documentacao',label:'Guia do sistema',icon:'book'},
      {route:'admin',label:'Administração',icon:'shield',children:[['users','Permissões'],['profiles','Perfis cadastrados'],['acessos','Alteração de acesso'],['bases','Configurações de bases'],['abas','Configurações de abas'],['alert-config','Configuração de alertas']]}
    ];
  ...
  function renderPermissoes(user){
    ...
    function renderPermList(perms){
      ...
      var buckets={usuarios:"users_","central-ocorrencias":"occ_",mecanicos:"mech_",passagem:"shift_",assistente:"ai_",ferias:"ferias_",sinistro:"sinistro_",ramais:"ramais_"};
  ...
  function openUserModal(id){
    ...
    var notifPrefs = u && u.notificationPrefs ? u.notificationPrefs : {sinistro:u ? u.notifySinistro !== false : true, ferias:true, sistema:true, alertas:true, chamados:true};
    ...
    document.getElementById("um-notif-chamados").checked = notifPrefs.chamados !== false;
  ...
  function readNotificationPrefs(){
    return {
      ...
      ,chamados: document.getElementById("um-notif-chamados").checked
    };
  }
  ...
  function renderClientAlerts(){
    ...
    root.innerHTML='<div class="page-header"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">'+icon('alert')+'</svg><h1>Alertas</h1><button class="back-btn" id="client-alert-back">← Voltar</button></div><main><div class="alerts-workspace"><section class="alerts-hero client-alerts-hero"><div><span class="eyebrow">RETORNO DA TRANSPORTADORA</span><h2>Solicitações aguardando sua resposta</h2><p>Abra o alerta, confira os dados enviados pela equipe operacional e informe a situação atual do veículo.</p></div><div class="alerts-summary client-alerts-summary"><article><span>Aguardando resposta</span><b>'+alerts.length+'</b></article><article class="critical"><span>Críticos</span><b>'+alerts.filter(function(alert){return alert.severity==='CRITICAL';}).length+'</b></article></div></section><section class="alert-tracking-panel"><div class="alerts-table-wrap"><table class="users-table alerts-table"><thead><tr><th>Evento</th><th>Veículo</th><th>Tipo</th><th>Criticidade</th><th>Situação</th><th>Ação</th></tr></thead><tbody id="client-alert-rows"></tbody></table></div></section></div></main>';
    document.getElementById('client-alert-back').onclick=function(){navigate('menu');};
    document.getElementById('client-alert-rows').innerHTML=alerts.map(function(a){return '<tr><td><b>'+escapeHtml(new Date(a.occurred_at).toLocaleString('pt-BR'))+'</b><br><small>'+escapeHtml(a.provider||'SMART RISK')+'</small></td><td><b>'+escapeHtml(a.plate||'—')+'</b><br><small>'+escapeHtml(a.driver_name||'Motorista não informado')+'</small></td><td>'+escapeHtml(a.alert_type||'—')+'</td><td><span class="severity-pill '+String(a.severity||'MEDIUM').toLowerCase()+'">'+escapeHtml(({LOW:'Baixa',MEDIUM:'Média',HIGH:'Alta',CRITICAL:'Crítica'})[a.severity]||a.severity||'Média')+'</span></td><td>'+escapeHtml(a.occurrence_summary||a.description||'Retorno solicitado')+'</td><td><button class="btn btn-primary btn-sm" data-client-reply="'+a.id+'">Responder</button></td></tr>';}).join('')||'<tr><td colspan="6" class="empty-state">Nenhum alerta aguardando resposta.</td></tr>';
    root.querySelectorAll('[data-client-reply]').forEach(function(button){
      button.onclick=function(){
        var alert=alerts.find(function(a){return a.id===button.dataset.clientReply;});
        if(!alert)return;
        document.body.insertAdjacentHTML('beforeend','<div class="modal-overlay show" id="client-response-modal"><div class="modal-card"><div class="modal-head"><div><small>RETORNO DA TRANSPORTADORA</small><h2>'+escapeHtml(alert.plate||'Alerta')+'</h2></div><button id="client-response-close">×</button></div><div class="record-detail-grid"><div><span>Tipo</span><b>'+escapeHtml(alert.alert_type||'—')+'</b></div><div><span>Data e hora</span><b>'+escapeHtml(new Date(alert.occurred_at).toLocaleString('pt-BR'))+'</b></div><div><span>Motorista</span><b>'+escapeHtml(alert.driver_name||'Não informado')+'</b></div><div><span>Localização</span><b>'+escapeHtml(alert.location_text||'Não informada')+'</b></div><div class="span-all"><span>Situação do veículo</span><b>'+escapeHtml(alert.occurrence_summary||alert.description||'Retorno solicitado pela operação.')+'</b></div></div><form id="client-response-form"><div class="fgroup"><label>Resposta / tratativa *</label><textarea id="client-response-text" minlength="3" rows="5" required placeholder="Informe o contato realizado e a situação atual"></textarea></div><div class="actions-row"><button type="button" class="btn btn-secondary" id="client-response-cancel">Cancelar</button><button type="button" class="btn btn-primary" id="client-response-submit">Enviar resposta</button></div></form></div></div>');
        var modal=document.getElementById('client-response-modal');
        var close=function(){modal.remove();};
        var sendResponse=async function(){
          var submit=document.getElementById('client-response-submit');
          var responseText=document.getElementById('client-response-text').value.trim();
          if(responseText.length<3){toast('Informe a resposta da transportadora.','error');return;}
          submit.disabled=true;
          var result=await supabaseClient.rpc('respond_tracking_alert',{requested_alert:alert.id,response_text:responseText});
          if(result.error){submit.disabled=false;toast(result.error.message,'error');return;}
          close();
          await loadCloudData();
          toast('Resposta enviada à equipe responsável.','success');
          renderClientAlerts();
        };
        document.getElementById('client-response-close').onclick=close;
        document.getElementById('client-response-cancel').onclick=close;
        document.getElementById('client-response-submit').onclick=sendResponse;
        document.getElementById('client-response-form').onsubmit=function(e){e.preventDefault();sendResponse();};
      };
    });
  }
  ...
  function renderCentralOcorrencias(){
    ...
    var main=document.querySelector("#view-root main"),legacyNodes=main?Array.from(main.children):[],trackingPanel=document.createElement("section");
    trackingPanel.className="alert-tracking-panel";
    if(main){
      main.innerHTML='<div class="alerts-workspace"><section class="alerts-hero"><div><span class="eyebrow">CENTRAL OPERACIONAL</span><h2>Acompanhamento de alertas</h2><p>Visualize os eventos recebidos, acompanhe prazos e registre as ações permitidas em cada etapa do atendimento.</p></div><div class="alerts-summary"><article><span>Em acompanhamento</span><b id="alerts-active-count">0</b></article><article><span>Aguardando cliente</span><b id="alerts-client-count">0</b></article><article class="critical"><span>Críticos</span><b id="alerts-critical-count">0</b></article><article><span>Respondidos</span><b id="alerts-responded-count">0</b></article></div></section><nav class="alerts-tabs" aria-label="Visualizações de alertas"><button type="button" class="alerts-tab active" data-alert-pane="integrator">Alertas do integrador</button><button type="button" class="alerts-tab" data-alert-pane="manual">Registros manuais</button></nav><section class="alerts-pane" id="alerts-pane-integrator"><div class="alerts-filterbar"><input id="tracking-alert-search" placeholder="Buscar por placa, motorista ou tipo..."><select id="tracking-alert-status"><option value="">Todos os status</option><option value="PENDING_TREATMENT">Pendente de tratamento</option><option value="IN_TREATMENT">Em tratamento</option><option value="WAITING_CLIENT">Aguardando cliente</option><option value="CLIENT_RESPONDED">Cliente respondeu</option><option value="TREATED">Tratado</option></select><select id="tracking-alert-severity"><option value="">Todas as criticidades</option><option value="LOW">Baixa</option><option value="MEDIUM">Média</option><option value="HIGH">Alta</option><option value="CRITICAL">Crítica</option></select></div><div id="tracking-alert-mount"></div></section><section class="alerts-pane" id="alerts-pane-manual" hidden></section></div>';
      document.getElementById('tracking-alert-mount').appendChild(trackingPanel);
      legacyNodes.forEach(function(node){document.getElementById('alerts-pane-manual').appendChild(node);});
      main.querySelectorAll('[data-alert-pane]').forEach(function(button){button.onclick=function(){main.querySelectorAll('[data-alert-pane]').forEach(function(item){item.classList.toggle('active',item===button);});document.getElementById('alerts-pane-integrator').hidden=button.dataset.alertPane!=="integrator";document.getElementById('alerts-pane-manual').hidden=button.dataset.alertPane!=="manual";};});
    }
    ...
    function workflowLabel(value){return ({PENDING_TREATMENT:"Pendente de tratamento",IN_TREATMENT:"Em tratamento",WAITING_CLIENT:"Aguardando cliente",CLIENT_RESPONDED:"Cliente respondeu",TREATED:"Tratado",DISCARDED:"Descartado",ARCHIVED:"Histórico"})[value]||String(value||"Pendente").replaceAll("_"," ");}
    function drawTrackingAlerts(){
      var allRows=(db.trackingAlerts||[]).filter(function(alert){return alert.status!=="ARCHIVED";}),query=(document.getElementById("tracking-alert-search")||{}).value||"",status=(document.getElementById("tracking-alert-status")||{}).value||"",severity=(document.getElementById("tracking-alert-severity")||{}).value||"";
      query=query.trim().toLocaleLowerCase("pt-BR");
      var rows=allRows.filter(function(alert){var text=[alert.plate,alert.driver_name,alert.alert_type,alert.provider,alert.description].join(" ").toLocaleLowerCase("pt-BR");return (!query||text.indexOf(query)!==-1)&&(!status||alert.status===status)&&(!severity||alert.severity===severity);});
      var setCount=function(id,value){var element=document.getElementById(id);if(element)element.textContent=value;};
      setCount("alerts-active-count",allRows.filter(function(alert){return ["TREATED","DISCARDED"].indexOf(alert.status)===-1;}).length);
      setCount("alerts-client-count",allRows.filter(function(alert){return alert.status==="WAITING_CLIENT";}).length);
      setCount("alerts-critical-count",allRows.filter(function(alert){return alert.severity==="CRITICAL"&&["TREATED","DISCARDED"].indexOf(alert.status)===-1;}).length);
      setCount("alerts-responded-count",allRows.filter(function(alert){return alert.status==="CLIENT_RESPONDED";}).length);
      trackingPanel.innerHTML='<div class="alerts-table-wrap"><table class="users-table alerts-table"><thead><tr><th>Evento</th><th>Veículo</th><th>Criticidade</th><th>Verificação</th><th>Contato</th><th>Workflow</th><th>Prazo</th><th>Ações</th></tr></thead><tbody>'+rows.map(function(alert){var canRequest=!!alert.transporter_id&&["PENDING_TREATMENT","IN_TREATMENT","CLIENT_RESPONDED"].indexOf(alert.status)!==-1,canTake=["PENDING_TREATMENT","CLIENT_RESPONDED"].indexOf(alert.status)!==-1,canTreat=["IN_TREATMENT","CLIENT_RESPONDED"].indexOf(alert.status)!==-1;return '<tr><td><div class="alert-primary"><b>'+escapeHtml(new Date(alert.occurred_at).toLocaleString("pt-BR"))+'</b><span>'+escapeHtml(alert.alert_type||"Evento")+'</span><small class="alert-origin">'+escapeHtml(alert.provider||"SMART RISK")+'</small></div></td><td><b>'+escapeHtml(alert.plate||"—")+'</b><br><small>'+escapeHtml(alert.driver_name||"Motorista não informado")+'</small></td><td><span class="severity-pill '+String(alert.severity||"MEDIUM").toLowerCase()+'">'+escapeHtml(({LOW:"Baixa",MEDIUM:"Média",HIGH:"Alta",CRITICAL:"Crítica"})[alert.severity]||alert.severity||"Média")+'</span></td><td>'+escapeHtml(occurrenceLabel(alert.occurrence_check_status))+'</td><td>'+escapeHtml(contactLabel(alert.contact_result))+'</td><td><span class="workflow-pill">'+escapeHtml(workflowLabel(alert.status))+'</span></td><td>'+escapeHtml(alert.deadline_at?new Date(alert.deadline_at).toLocaleString("pt-BR"):"Sem prazo")+'</td><td><div class="row-actions"><button data-tracking-view="'+alert.id+'">Detalhes</button>'+(canTake?'<button data-tracking-take="'+alert.id+'">Assumir</button>':'')+(canRequest?'<button data-tracking-client="'+alert.id+'">Solicitar retorno</button>':'')+(canTreat?'<button data-tracking-treat="'+alert.id+'">Concluir</button>':'')+'</div></td></tr>';}).join("")+'</tbody></table>'+(rows.length?'':'<div class="empty-state">Nenhum alerta corresponde aos filtros selecionados.</div>')+'</div>';
      trackingPanel.querySelectorAll("[data-tracking-view]").forEach(function(button){button.onclick=function(){var alert=rows.find(function(item){return item.id===button.dataset.trackingView;});if(!alert)return;document.body.insertAdjacentHTML("beforeend",'<div class="modal-overlay show" id="tracking-detail"><div class="modal-card"><div class="modal-head"><div><small>ALERTA DO INTEGRADOR</small><h2>'+escapeHtml(alert.plate||"Alerta")+'</h2></div><button data-track-close>×</button></div><div class="record-detail-grid"><div><span>Tipo</span><b>'+escapeHtml(alert.alert_type||"Não informado")+'</b></div><div><span>Status</span><b>'+escapeHtml(workflowLabel(alert.status))+'</b></div><div><span>Criticidade</span><b>'+escapeHtml(alert.severity||"Não informada")+'</b></div><div><span>Verificação automática</span><b>'+escapeHtml(occurrenceLabel(alert.occurrence_check_status))+'</b></div><div><span>Contato</span><b>'+escapeHtml(contactLabel(alert.contact_result))+'</b></div><div><span>Motorista</span><b>'+escapeHtml(alert.driver_name||"Não informado")+'</b></div><div><span>Origem</span><b>'+escapeHtml(alert.provider||"Smart Risk")+'</b></div><div><span>Data do evento</span><b>'+escapeHtml(new Date(alert.occurred_at).toLocaleString("pt-BR"))+'</b></div><div class="span-all"><span>Descrição do integrador</span><p>'+escapeHtml(alert.description||"Sem descrição")+'</p></div><div class="span-all"><span>Ocorrência localizada pelo sistema</span><p>'+escapeHtml(alert.occurrence_summary||"O sistema ainda não localizou uma ocorrência correspondente.")+'</p></div>'+(alert.client_response?'<div class="span-all"><span>Retorno da transportadora</span><p>'+escapeHtml(alert.client_response)+'</p></div>':'')+(alert.treatment_description?'<div class="span-all"><span>Tratamento</span><p>'+escapeHtml(alert.treatment_description)+'</p></div>':'')+'</div></div></div>');document.querySelector("[data-track-close]").onclick=function(){document.getElementById("tracking-detail").remove();};};});
      trackingPanel.querySelectorAll("[data-tracking-client]").forEach(function(button){button.onclick=function(){var alert=rows.find(function(item){return item.id===button.dataset.trackingClient;});if(!alert)return;document.body.insertAdjacentHTML("beforeend",'<div class="modal-overlay show" id="tracking-client-modal"><div class="modal-card"><div class="modal-head"><div><small>SOLICITAR RETORNO</small><h2>'+escapeHtml(alert.plate||"Alerta")+'</h2></div><button data-track-client-close>×</button></div><form id="tracking-client-form"><div class="fgroup"><label>Situação apresentada à transportadora *</label><textarea id="tracking-client-summary" required minlength="3" rows="5" placeholder="Revise a situação localizada pelo sistema e descreva o retorno necessário.">'+escapeHtml(alert.occurrence_summary||alert.description||"")+'</textarea></div><div class="actions-row"><button type="button" class="btn btn-secondary" data-track-client-close>Cancelar</button><button class="btn btn-primary">Enviar ao cliente</button></div></form></div></div>');function close(){document.getElementById("tracking-client-modal").remove();}document.querySelectorAll("[data-track-client-close]").forEach(function(item){item.onclick=close;});document.getElementById("tracking-client-form").onsubmit=async function(event){event.preventDefault();var result=await supabaseClient.rpc("request_client_tracking_response",{requested_alert:alert.id,situation_summary:document.getElementById("tracking-client-summary").value.trim()});if(result.error){toast(result.error.message,"error");return;}close();await loadCloudData();drawTrackingAlerts();toast("Solicitação enviada à transportadora.","success");};};});
      ...
      trackingPanel.querySelectorAll("[data-tracking-treat]").forEach(function(button){button.onclick=function(){var alert=rows.find(function(item){return item.id===button.dataset.trackingTreat;});if(!alert)return;document.body.insertAdjacentHTML("beforeend",'<div class="modal-overlay show" id="tracking-treat-modal"><div class="modal-card"><div class="modal-head"><div><small>CONCLUIR TRATAMENTO</small><h2>'+escapeHtml(alert.plate||"Alerta")+'</h2></div><button data-track-treat-close>×</button></div><form id="tracking-treat-form"><div class="fgroup"><label>Descrição do tratamento *</label><textarea id="tracking-treatment-text" required minlength="3" rows="5"></textarea></div><div class="actions-row"><button type="button" class="btn btn-secondary" data-track-treat-close>Cancelar</button><button class="btn btn-primary">Concluir tratamento</button></div></form></div></div>');function close(){document.getElementById("tracking-treat-modal").remove();}document.querySelectorAll("[data-track-treat-close]").forEach(function(item){item.onclick=close;});document.getElementById("tracking-treat-form").onsubmit=async function(event){event.preventDefault();var result=await supabaseClient.rpc("treat_tracking_alert",{requested_alert:alert.id,treatment_text:document.getElementById("tracking-treatment-text").value.trim()});if(result.error){toast(result.error.message,"error");return;}close();await loadCloudData();drawTrackingAlerts();toast("Alerta tratado. Ele permanecerá visível por 10 minutos.","success");};};});
    }
    drawTrackingAlerts();
    ["tracking-alert-search","tracking-alert-status","tracking-alert-severity"].forEach(function(id){var control=document.getElementById(id);if(control){control.oninput=drawTrackingAlerts;control.onchange=drawTrackingAlerts;}});
  ...
  function renderBaseExtensions(){
    var root=document.getElementById("base-extensions-root"),user=currentUser(),canManage=!!userPerms(user).ramais_manage;
    if(!root)return;
    root.innerHTML='<section class="module-intro"><div><span class="eyebrow">DIRETÓRIO DE CONTATOS</span><h2>Ramais das bases</h2><p>Consulte os contatos por transportadora, base, setor ou número.</p></div></section><nav class="workspace-tabs" aria-label="Áreas de ramais"><button type="button" class="workspace-tab active" data-extension-pane="search">Pesquisar ramais</button>'+(canManage?'<button type="button" class="workspace-tab" data-extension-pane="manage">Cadastrar ramal</button>':'')+'</nav><section id="extension-search-pane" class="workspace-pane"><div class="directory-toolbar"><label><span>Pesquisa</span><input id="extension-search" type="search" placeholder="Transportadora, base, setor ou ramal"></label><div class="directory-count"><b id="extension-result-count">0</b><span>resultados</span></div></div><div class="data-table-shell"><table class="users-table directory-table"><thead><tr><th>Base</th><th>Transportadoras vinculadas</th><th>Setor</th><th>Ramal</th>'+(canManage?'<th>Ações</th>':'')+'</tr></thead><tbody id="extension-rows"></tbody></table></div></section>'+(canManage?'<section id="extension-manage-pane" class="workspace-pane" hidden><div class="section-card form-section"><div class="section-title"><h2>Novo ramal</h2><p class="hint-text">O ramal ficará disponível imediatamente na pesquisa para todos os usuários autorizados.</p></div><form id="extension-form"><div class="form-grid three"><div class="fgroup"><label>Base *</label><select id="extension-base" required><option value="">Selecione a base</option>'+db.operationalBases.map(function(base){return '<option value="'+base.id+'">'+escapeHtml(base.name)+' · '+escapeHtml(base.code)+'</option>';}).join('')+'</select></div><div class="fgroup"><label>Setor / identificação *</label><input id="extension-label" maxlength="80" required placeholder="Ex.: Operação 24h"></div><div class="fgroup"><label>Número do ramal *</label><input id="extension-number" maxlength="30" required placeholder="Ex.: 204"></div></div><div class="form-actions"><button class="btn btn-primary" id="extension-submit">Cadastrar ramal</button></div></form></div></section>':'');
    function transporterNames(baseId){return db.baseTransporters.filter(function(link){return link.base_id===baseId;}).map(function(link){return (db.transporters.find(function(item){return item.id===link.transporter_id;})||{}).name;}).filter(Boolean);}
    function draw(){var input=document.getElementById('extension-search'),query=String(input&&input.value||'').trim().toLocaleLowerCase('pt-BR'),rows=(db.baseExtensions||[]).map(function(extension){var base=db.operationalBases.find(function(item){return item.id===extension.base_id;}),transporters=transporterNames(extension.base_id);return {extension:extension,base:base,transporters:transporters,haystack:[base&&base.name,base&&base.code,extension.label,extension.extension_number].concat(transporters).join(' ').toLocaleLowerCase('pt-BR')};}).filter(function(row){return row.base&&(!query||row.haystack.indexOf(query)!==-1);});document.getElementById('extension-result-count').textContent=rows.length;document.getElementById('extension-rows').innerHTML=rows.map(function(row){return '<tr><td><b>'+escapeHtml(row.base.name)+'</b><br><small>'+escapeHtml(row.base.code)+'</small></td><td>'+escapeHtml(row.transporters.join(', ')||'Sem transportadora vinculada')+'</td><td>'+escapeHtml(row.extension.label)+'</td><td><strong class="extension-value">'+escapeHtml(row.extension.extension_number)+'</strong></td>'+(canManage?'<td><button class="table-action danger" data-extension-remove="'+row.extension.id+'">Desativar</button></td>':'')+'</tr>';}).join('')||'<tr><td colspan="'+(canManage?5:4)+'" class="empty-state">Nenhum ramal corresponde à pesquisa.</td></tr>';root.querySelectorAll('[data-extension-remove]').forEach(function(button){button.onclick=async function(){if(!confirm('Desativar este ramal?'))return;var result=await supabaseClient.from('base_extensions').update({active:false,updated_at:new Date().toISOString()}).eq('id',button.dataset.extensionRemove);if(result.error){toast(result.error.message,'error');return;}await loadCloudData();draw();toast('Ramal desativado.','success');};});}
    root.querySelectorAll('[data-extension-pane]').forEach(function(button){button.onclick=function(){root.querySelectorAll('[data-extension-pane]').forEach(function(item){item.classList.toggle('active',item===button);});document.getElementById('extension-search-pane').hidden=button.dataset.extensionPane!=='search';var manage=document.getElementById('extension-manage-pane');if(manage)manage.hidden=button.dataset.extensionPane!=='manage';};});
    document.getElementById('extension-search').oninput=draw;
    var form=document.getElementById('extension-form');if(form)form.onsubmit=async function(event){event.preventDefault();if(!supabaseClient||!db.supabaseConnected){toast('Conecte o SmartRisk ao Supabase.','error');return;}var submit=document.getElementById('extension-submit');submit.disabled=true;submit.textContent='Cadastrando...';try{var result=await supabaseClient.from('base_extensions').insert({base_id:document.getElementById('extension-base').value,label:document.getElementById('extension-label').value.trim(),extension_number:document.getElementById('extension-number').value.trim(),created_by:user.id});if(result.error)throw result.error;form.reset();await loadCloudData();draw();root.querySelector('[data-extension-pane="search"]').click();toast('Ramal cadastrado.','success');}catch(error){toast(error.message||'Não foi possível cadastrar o ramal.','error');}finally{submit.disabled=false;submit.textContent='Cadastrar ramal';}};
    draw();
  }

  function supportStatusLabel(status){return {OPEN:'Aberto',IN_PROGRESS:'Em atendimento',WAITING_USER:'Aguardando usuário',RESOLVED:'Resolvido',CLOSED:'Encerrado'}[status]||status;}
  function supportPriorityLabel(priority){return {LOW:'Baixa',NORMAL:'Normal',HIGH:'Alta',URGENT:'Urgente'}[priority]||priority;}
  function supportTicketText(ticket){var requester=db.users.find(function(user){return user.id===ticket.requester_id;}),assignee=db.users.find(function(user){return user.id===ticket.assigned_to;});return ['SMART RISK · TICKET DE ATENDIMENTO','',ticket.ticket_number,'Status: '+supportStatusLabel(ticket.status),'Prioridade: '+supportPriorityLabel(ticket.priority),'Categoria: '+ticket.category,'Solicitante: '+(requester?requester.nome:'Usuário'),'Responsável: '+(assignee?assignee.nome:'Não atribuído'),'Aberto em: '+new Date(ticket.created_at).toLocaleString('pt-BR'),'Atualizado em: '+new Date(ticket.updated_at||ticket.created_at).toLocaleString('pt-BR'),'','ASSUNTO',ticket.subject,'','DESCRIÇÃO',ticket.description,'','RETORNO DA EQUIPE',ticket.response_text||'Ainda não há retorno registrado.'].join('\n');}
  function generateSupportTicket(ticket){var content=escapeHtml(supportTicketText(ticket)).replaceAll('\n','<br>'),frame=document.createElement('iframe');frame.setAttribute('title','Ticket '+ticket.ticket_number);frame.style.cssText='position:fixed;left:-10000px;top:0;width:800px;height:1000px;border:0;opacity:0;pointer-events:none';document.body.appendChild(frame);var documentRef=frame.contentDocument;documentRef.open();documentRef.write('<!doctype html><html><head><meta charset="utf-8"><title>'+escapeHtml(ticket.ticket_number)+'</title><style>body{font:14px Arial,sans-serif;color:#182230;margin:42px;line-height:1.55}header{border-bottom:2px solid #163d5a;padding-bottom:14px;margin-bottom:22px}h1{font-size:20px;margin:0}p{white-space:normal}.ticket{max-width:760px}.content{border:1px solid #d0d5dd;padding:20px}</style></head><body><div class="ticket"><header><h1>Smart Risk · Ticket de atendimento</h1></header><div class="content">'+content+'</div></div></body></html>');documentRef.close();setTimeout(function(){frame.contentWindow.focus();frame.contentWindow.print();setTimeout(function(){frame.remove();},1000);},120);}
  function openSupportTicketDetails(ticket){var requester=db.users.find(function(user){return user.id===ticket.requester_id;}),assignee=db.users.find(function(user){return user.id===ticket.assigned_to;});document.getElementById('support-detail-modal')?.remove();document.body.insertAdjacentHTML('beforeend','<div class="modal-overlay show" id="support-detail-modal"><div class="modal-card support-detail-card"><div class="modal-head"><div><small>'+escapeHtml(ticket.ticket_number)+'</small><h2>'+escapeHtml(ticket.subject)+'</h2></div><button type="button" data-ticket-detail-close aria-label="Fechar">×</button></div><div class="ticket-detail-status"><span class="ticket-status status-'+String(ticket.status).toLowerCase()+'">'+escapeHtml(supportStatusLabel(ticket.status))+'</span><span class="ticket-priority priority-'+String(ticket.priority||'NORMAL').toLowerCase()+'">Prioridade '+escapeHtml(supportPriorityLabel(ticket.priority))+'</span></div><dl class="ticket-detail-meta"><div><dt>Categoria</dt><dd>'+escapeHtml(ticket.category)+'</dd></div><div><dt>Solicitante</dt><dd>'+escapeHtml(requester?requester.nome:'Usuário')+'</dd></div><div><dt>Responsável</dt><dd>'+escapeHtml(assignee?assignee.nome:'Não atribuído')+'</dd></div><div><dt>Aberto em</dt><dd>'+new Date(ticket.created_at).toLocaleString('pt-BR')+'</dd></div></dl><section class="ticket-detail-section"><h3>Descrição da solicitação</h3><p>'+escapeHtml(ticket.description)+'</p></section><section class="ticket-detail-section"><h3>Retorno da equipe</h3><p>'+escapeHtml(ticket.response_text||'Ainda não há retorno registrado para este chamado.')+'</p></section><div class="modal-footer-actions"><button type="button" class="btn btn-secondary" data-ticket-detail-close>Fechar</button><button type="button" class="btn btn-primary" id="support-detail-generate">Gerar ticket</button></div></div></div>');var modal=document.getElementById('support-detail-modal');modal.querySelectorAll('[data-ticket-detail-close]').forEach(function(button){button.onclick=function(){modal.remove();};});document.getElementById('support-detail-generate').onclick=function(){generateSupportTicket(ticket);};}
  function supportTicketCard(ticket,management){var requester=db.users.find(function(user){return user.id===ticket.requester_id;}),assignee=db.users.find(function(user){return user.id===ticket.assigned_to;});return '<article class="support-ticket-row" data-ticket-id="'+ticket.id+'"><div class="support-ticket-main"><div class="support-ticket-id"><b>'+escapeHtml(ticket.ticket_number)+'</b><span>'+new Date(ticket.created_at).toLocaleString('pt-BR')+'</span></div><div class="support-ticket-subject"><h3>'+escapeHtml(ticket.subject)+'</h3><p>'+escapeHtml(ticket.category)+' · '+escapeHtml(requester?requester.nome:'Usuário')+'</p></div><div class="support-ticket-owner"><span>Responsável</span><b>'+escapeHtml(assignee?assignee.nome:'Não atribuído')+'</b></div><span class="ticket-priority priority-'+String(ticket.priority||'NORMAL').toLowerCase()+'">'+escapeHtml(supportPriorityLabel(ticket.priority))+'</span><span class="ticket-status status-'+String(ticket.status).toLowerCase()+'">'+escapeHtml(supportStatusLabel(ticket.status))+'</span></div><div class="support-ticket-actions"><button type="button" class="table-action" data-ticket-view="'+ticket.id+'">Ver chamado</button><button type="button" class="table-action" data-ticket-generate="'+ticket.id+'">Gerar ticket</button>'+(management?'<button type="button" class="table-action primary" data-ticket-manage="'+ticket.id+'">Tratar</button>':'')+'</div></article>';}
  function bindSupportTicketActions(root,rows){root.querySelectorAll('[data-ticket-view]').forEach(function(button){button.onclick=function(){var ticket=rows.find(function(item){return item.id===button.dataset.ticketView;});if(ticket)openSupportTicketDetails(ticket);};});root.querySelectorAll('[data-ticket-generate]').forEach(function(button){button.onclick=function(){var ticket=rows.find(function(item){return item.id===button.dataset.ticketGenerate;});if(ticket)generateSupportTicket(ticket);};});}

  function renderSupportRequest(){
    var root=document.getElementById('support-request-root'),user=currentUser();if(!root)return;
    root.innerHTML='<section class="module-intro"><div><span class="eyebrow">CENTRAL DE SUPORTE</span><h2>Abertura de chamados</h2><p>Registre uma solicitação e acompanhe a análise, o responsável e o retorno da equipe.</p></div></section><div class="support-workspace"><section class="section-card support-form-card"><div class="section-title"><h2>Novo chamado</h2><p class="hint-text">Informe o contexto completo para reduzir o tempo de atendimento.</p></div><form id="support-request-form"><div class="form-grid two"><div class="fgroup"><label>Tipo de solicitação *</label><select id="support-category" required><option value="">Selecione</option><option>Criação de novo usuário</option><option>Dúvida sobre o sistema</option><option>Problema técnico</option><option>Solicitação de acesso</option><option>Correção de cadastro</option><option>Outro tipo de solicitação</option></select></div><div class="fgroup"><label>Prioridade *</label><select id="support-priority"><option value="LOW">Baixa</option><option value="NORMAL" selected>Normal</option><option value="HIGH">Alta</option><option value="URGENT">Urgente</option></select></div><div class="fgroup span-all"><label>Assunto *</label><input id="support-subject" required maxlength="160" placeholder="Resumo objetivo da solicitação"></div><div class="fgroup span-all"><label>Descrição detalhada *</label><textarea id="support-description" required minlength="10" maxlength="4000" rows="7" placeholder="Descreva o problema, o resultado esperado e as informações necessárias para análise."></textarea><small class="field-help">Mínimo de 10 caracteres. Não inclua senhas ou chaves de acesso.</small></div></div><div class="form-actions"><button class="btn btn-primary" id="support-submit">Abrir chamado</button></div></form></section><section class="section-card support-history-card"><div class="section-title"><h2>Meus chamados</h2><p class="hint-text">Abra um chamado para consultar a descrição completa e o retorno recebido.</p></div><div id="support-my-list" class="support-ticket-list"></div></section></div>';
    function draw(){var rows=(db.supportTickets||[]).filter(function(ticket){return ticket.requester_id===user.id;});document.getElementById('support-my-list').innerHTML=rows.map(function(ticket){return supportTicketCard(ticket,false);}).join('')||'<div class="empty-state">Você ainda não abriu chamados.</div>';bindSupportTicketActions(root,rows);}
    document.getElementById('support-request-form').onsubmit=async function(event){event.preventDefault();if(!supabaseClient||!db.supabaseConnected){toast('Conecte o SmartRisk ao Supabase.','error');return;}var button=document.getElementById('support-submit');button.disabled=true;button.textContent='Enviando...';try{var result=await supabaseClient.rpc('create_support_ticket',{ticket_category:document.getElementById('support-category').value,ticket_subject:document.getElementById('support-subject').value.trim(),ticket_description:document.getElementById('support-description').value.trim(),ticket_priority:document.getElementById('support-priority').value});if(result.error)throw result.error;event.target.reset();await loadCloudData();draw();toast('Chamado aberto. O número do ticket já está disponível em Meus chamados.','success');refreshNotifBadge();}catch(error){console.error(error);toast(error.message||'Não foi possível abrir o chamado.','error');}finally{button.disabled=false;button.textContent='Abrir chamado';}};
    draw();
  }

  function renderSupportManagement(){
    var root=document.getElementById('support-management-root'),user=currentUser();if(!root)return;
    root.innerHTML='<section class="module-intro support-queue-intro"><div><span class="eyebrow">ATENDIMENTO INTERNO</span><h2>Fila de chamados</h2><p>Consulte a solicitação completa, atribua um responsável e registre o retorno.</p></div><div class="support-metrics"><div><strong id="support-open-count">0</strong><span>Em aberto</span></div><div><strong id="support-progress-count">0</strong><span>Em atendimento</span></div></div></section><section class="section-card"><div class="support-filters"><input id="support-search" type="search" placeholder="Número, solicitante, assunto ou descrição"><select id="support-status-filter"><option value="">Todos os status</option><option value="OPEN">Aberto</option><option value="IN_PROGRESS">Em atendimento</option><option value="WAITING_USER">Aguardando usuário</option><option value="RESOLVED">Resolvido</option><option value="CLOSED">Encerrado</option></select></div><div id="support-management-list" class="support-ticket-list"></div></section><div class="modal-overlay" id="support-manage-modal"><div class="modal-card support-manage-card"><div class="modal-head"><div><small>TRATAMENTO DO CHAMADO</small><h2 id="support-manage-title"></h2></div><button type="button" id="support-manage-close" aria-label="Fechar">×</button></div><div class="support-manage-context"><span>Descrição da solicitação</span><p id="support-manage-description"></p></div><form id="support-manage-form"><input type="hidden" id="support-manage-id"><div class="form-grid two"><div class="fgroup"><label>Status *</label><select id="support-manage-status"><option value="IN_PROGRESS">Em atendimento</option><option value="WAITING_USER">Aguardando usuário</option><option value="RESOLVED">Resolvido</option><option value="CLOSED">Encerrado</option></select></div><div class="fgroup"><label>Responsável</label><select id="support-manage-assignee"><option value="">Manter sem responsável</option>'+db.users.filter(function(candidate){return userPerms(candidate).chamados;}).map(function(candidate){return '<option value="'+candidate.id+'">'+escapeHtml(candidate.nome)+'</option>';}).join('')+'</select></div><div class="fgroup span-all"><label>Retorno ao solicitante</label><textarea id="support-manage-response" rows="6" maxlength="4000" placeholder="Registre a análise, orientação ou solução aplicada."></textarea></div></div><div class="modal-footer-actions"><button type="button" class="btn btn-secondary" id="support-manage-cancel">Cancelar</button><button class="btn btn-primary" id="support-manage-submit">Salvar tratamento</button></div></form></div></div>';
    function draw(){var query=document.getElementById('support-search').value.trim().toLocaleLowerCase('pt-BR'),status=document.getElementById('support-status-filter').value,rows=(db.supportTickets||[]).filter(function(ticket){var requester=db.users.find(function(item){return item.id===ticket.requester_id;}),text=[ticket.ticket_number,ticket.subject,ticket.description,requester&&requester.nome].join(' ').toLocaleLowerCase('pt-BR');return (!query||text.indexOf(query)!==-1)&&(!status||ticket.status===status);});document.getElementById('support-open-count').textContent=(db.supportTickets||[]).filter(function(ticket){return ticket.status==='OPEN';}).length;document.getElementById('support-progress-count').textContent=(db.supportTickets||[]).filter(function(ticket){return ['IN_PROGRESS','WAITING_USER'].indexOf(ticket.status)!==-1;}).length;document.getElementById('support-management-list').innerHTML=rows.map(function(ticket){return supportTicketCard(ticket,true);}).join('')||'<div class="empty-state">Nenhum chamado encontrado.</div>';bindSupportTicketActions(root,rows);root.querySelectorAll('[data-ticket-manage]').forEach(function(button){button.onclick=function(){var ticket=rows.find(function(item){return item.id===button.dataset.ticketManage;});if(!ticket)return;document.getElementById('support-manage-id').value=ticket.id;document.getElementById('support-manage-title').textContent=ticket.ticket_number+' · '+ticket.subject;document.getElementById('support-manage-description').textContent=ticket.description;document.getElementById('support-manage-status').value=ticket.status==='OPEN'?'IN_PROGRESS':ticket.status;document.getElementById('support-manage-assignee').value=ticket.assigned_to||user.id;document.getElementById('support-manage-response').value=ticket.response_text||'';document.getElementById('support-manage-modal').classList.add('show');};});}
    function close(){document.getElementById('support-manage-modal').classList.remove('show');}
    document.getElementById('support-search').oninput=draw;document.getElementById('support-status-filter').onchange=draw;document.getElementById('support-manage-close').onclick=close;document.getElementById('support-manage-cancel').onclick=close;
    document.getElementById('support-manage-form').onsubmit=async function(event){event.preventDefault();var button=document.getElementById('support-manage-submit');button.disabled=true;button.textContent='Salvando...';try{var result=await supabaseClient.rpc('manage_support_ticket',{requested_ticket:document.getElementById('support-manage-id').value,new_status:document.getElementById('support-manage-status').value,ticket_response:document.getElementById('support-manage-response').value.trim()||null,assigned_user:document.getElementById('support-manage-assignee').value||null});if(result.error)throw result.error;close();await loadCloudData();draw();toast('Chamado atualizado.','success');}catch(error){console.error(error);toast(error.message||'Não foi possível atualizar o chamado.','error');}finally{button.disabled=false;button.textContent='Salvar tratamento';}};
    draw();
  }

  function renderConfigAbas(){
    var root=document.getElementById("config-abas-v2-root");if(!root)return;
    var sections=[
      {key:"plantoes",title:"Plantões",description:"Turnos disponíveis nos registros de efetivo e faltas.",icon:"clock"},
      {key:"transferenciaMotivos",title:"Motivos de transferência",description:"Opções usadas nas movimentações operacionais.",icon:"shift"},
      {key:"clientes",title:"Operações e clientes",description:"Nomes utilizados nos formulários e relatórios operacionais.",icon:"org"},
      {key:"sinistroTipos",title:"Tipos de sinistro",description:"Classificações disponíveis no registro de sinistros.",icon:"file"},
      {key:"viagemTipos",title:"Tipos de viagem",description:"Modalidades utilizadas nos registros de viagem.",icon:"truck"},
      {key:"tecnologias",title:"Tecnologias",description:"Rastreadores e tecnologias disponíveis nos formulários.",icon:"settings"},
      {key:"deslocamentoMotivos",title:"Motivos de pronta resposta",description:"Motivos disponíveis nos acionamentos de pronta resposta.",icon:"alert"}
    ],active=null;
    root.innerHTML='<section class="config-hub" id="config-list-home"><div class="choice-hub-intro"><span class="eyebrow">LISTAS DO SISTEMA</span><h2>O que deseja configurar?</h2><p class="hint-text">Escolha uma lista para adicionar, revisar ou remover opções usadas nas telas do SmartRisk.</p></div><div class="config-card-grid">'+sections.map(function(section){return '<button class="config-card" data-config-section="'+section.key+'"><span class="config-card-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">'+icon(section.icon)+'</svg></span><span><b>'+escapeHtml(section.title)+'</b><small>'+escapeHtml(section.description)+'</small></span></button>';}).join('')+'</div></section><section class="section-card config-editor" id="config-list-editor" hidden><button type="button" class="subpage-back" id="config-list-back">← Voltar às configurações</button><div class="section-title"><span class="eyebrow">CONFIGURAÇÃO DE LISTA</span><h2 id="config-list-title"></h2><p class="hint-text" id="config-list-description"></p></div><form id="config-list-form" class="config-toolbar"><div class="fgroup"><label>Nova opção</label><input id="config-list-value" required maxlength="160" placeholder="Digite o nome da opção"></div><button class="btn btn-primary">Adicionar</button></form><div id="config-list-items" class="config-list"></div></section>';
    function draw(){var section=sections.find(function(item){return item.key===active;}),list=section&&Array.isArray(db.tabSettings[section.key])?db.tabSettings[section.key]:[];if(!section)return;document.getElementById('config-list-title').textContent=section.title;document.getElementById('config-list-description').textContent=section.description;document.getElementById('config-list-items').innerHTML=list.map(function(value,index){return '<div class="config-item"><span>'+escapeHtml(value)+'</span><button type="button" class="btn btn-danger btn-sm" data-config-remove="'+index+'">Remover</button></div>';}).join('')||'<div class="empty-state config-empty">Nenhuma opção cadastrada.</div>';root.querySelectorAll('[data-config-remove]').forEach(function(button){button.onclick=function(){db.tabSettings[section.key].splice(Number(button.dataset.configRemove),1);saveDB(db);draw();toast('Opção removida.','success');};});}
    function open(key){active=key;document.getElementById('config-list-home').hidden=true;document.getElementById('config-list-editor').hidden=false;draw();setTimeout(function(){document.getElementById('config-list-value').focus();},20);}
    root.querySelectorAll('[data-config-section]').forEach(function(button){button.onclick=function(){open(button.dataset.configSection);};});
    document.getElementById('config-list-back').onclick=function(){active=null;document.getElementById('config-list-editor').hidden=true;document.getElementById('config-list-home').hidden=false;};
    document.getElementById('config-list-form').onsubmit=function(event){event.preventDefault();var section=sections.find(function(item){return item.key===active;}),input=document.getElementById('config-list-value'),value=input.value.trim();if(!section||!value)return;var list=db.tabSettings[section.key]||(db.tabSettings[section.key]=[]);if(list.some(function(item){return item.toLocaleLowerCase('pt-BR')===value.toLocaleLowerCase('pt-BR');})){toast('Esta opção já está cadastrada.','error');return;}list.push(value);list.sort(function(a,b){return a.localeCompare(b,'pt-BR');});saveDB(db);input.value='';draw();toast('Opção adicionada.','success');};
  }

  function renderAlertConfiguration(){
    var root=document.getElementById("alert-config-root");
    if(!root)return;
    root.innerHTML='<section class="alert-config-intro"><div><span class="eyebrow">REGRAS DE ALERTA</span><h2>Tipos e criticidade</h2><p>Configure a criticidade visual e as regras de encerramento dos alertas registrados no SmartRisk.</p></div></section><div id="internal-alert-rules"></div><section class="section-card" style="margin-top:16px"><div class="section-title"><span class="eyebrow">INTEGRAÇÃO DE RASTREAMENTO</span><h2>Alertas recebidos do integrador</h2><p class="hint-text">Mapeie os códigos externos para os tipos internos. Esta configuração permanece separada das demais listas do sistema.</p></div><form id="tracking-mapping-form"><div class="form-grid three"><div class="fgroup"><label>Código no fornecedor *</label><input id="mapping-code" maxlength="100" required></div><div class="fgroup"><label>Nome no fornecedor *</label><input id="mapping-name" maxlength="160" required></div><div class="fgroup"><label>Tipo normalizado *</label><select id="mapping-type" required><option value="">Selecione</option><option>PANIC_BUTTON</option><option>TRAILER_DETACHMENT</option><option>VIOLATION</option><option>ROUTE_DEVIATION</option><option>RISK_AREA_ENTRY</option><option>RISK_AREA_EXIT</option><option>COMMUNICATION_LOSS</option><option>GPS_LOSS</option><option>IGNITION</option><option>DOOR_OPEN</option><option>TRACKING_EVENT</option><option>OTHER</option></select></div><div class="fgroup"><label>Severidade *</label><select id="mapping-severity"><option>LOW</option><option selected>MEDIUM</option><option>HIGH</option><option>CRITICAL</option></select></div><div class="fgroup"><label>Prioridade *</label><select id="mapping-priority"><option>LOW</option><option selected>NORMAL</option><option>HIGH</option><option>URGENT</option></select></div><div class="fgroup"><label>Descrição</label><input id="mapping-description" maxlength="300"></div></div><div class="form-grid three"><label class="setting-toggle"><span><b>Ativo</b><small>Considerar eventos deste código</small></span><input id="mapping-active" type="checkbox" checked></label><label class="setting-toggle"><span><b>Gerar alerta</b><small>Criar alerta operacional</small></span><input id="mapping-occurrence" type="checkbox" checked></label><label class="setting-toggle"><span><b>Necessita tratamento</b><small>Manter prazo e escalonamento ativos</small></span><input id="mapping-treatment" type="checkbox" checked></label><label class="setting-toggle"><span><b>Usar Groq</b><small>Somente se houver texto não estruturado</small></span><input id="mapping-ai" type="checkbox"></label></div><div class="fgroup"><label>Instruções adicionais para a IA</label><textarea id="mapping-ai-instructions" rows="3" maxlength="1000" placeholder="Opcional. Não inclua regras de prazo, escalonamento ou permissão."></textarea></div><button class="btn btn-primary">Salvar mapeamento</button></form></section><section class="section-card" style="margin-top:16px"><div class="section-title"><h2>Mapeamentos cadastrados</h2><p class="hint-text">O provider de teste usa estes mesmos códigos; depois, o adaptador real poderá ser trocado sem alterar o workflow.</p></div><div id="tracking-mapping-list" class="record-grid compact-records"></div></section>';
    CSRWorkspace.alertRules(document.getElementById('internal-alert-rules'));
  ...
  function addNotification(targetUserId, type, title, message, route, recordId){
    ...
    if(targetUser){
      ...
      var prefs = targetUser.notificationPrefs || {sinistro:targetUser.notifySinistro !== false, ferias:true, sistema:true, alertas:true, chamados:true};
