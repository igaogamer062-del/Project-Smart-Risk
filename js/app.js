...
(function(){
  ...
  var DEFAULT_PERMS = {
    Administrador:{menu:true,perfil:true,permissoes:true,usuarios:true,dashboard:true,ocorrencia:true,assistente:true,centralOcorrencias:true,passagem:true,sinistro:true,efetivo:true,checklist:true,ferias:true,configAbas:true,ramais:true,abrirChamado:true,chamados:true,logs:true},
  ...
  function renderBaseExtensions(){
    var root=document.getElementById("base-extensions-root"),user=currentUser(),isAdmin=CSRRules.role(user)==="Administrador";
    if(!root)return;
    root.innerHTML='<section class="directory-hero"><div><span class="eyebrow">DIRETÓRIO OPERACIONAL</span><h2>Encontre rapidamente a base e o ramal</h2><p>Pesquise pelo nome da transportadora, base, setor ou número do ramal.</p></div><label class="directory-search"><span>Pesquisar transportadora ou base</span><input id="extension-search" type="search" placeholder="Ex.: LEMAR, Diversos, Operação ou 204"></label></section>'+(isAdmin?'<section class="section-card extension-admin-card"><div class="section-title"><h2>Cadastrar ramal</h2><p class="hint-text">Cada ramal é vinculado a uma base configurada no SmartRisk.</p></div><form id="extension-form"><div class="form-grid three"><div class="fgroup"><label>Base *</label><select id="extension-base" required><option value="">Selecione a base</option>'+db.operationalBases.map(function(base){return '<option value="'+base.id+'">'+escapeHtml(base.name)+'</option>';}).join('')+'</select></div><div class="fgroup"><label>Setor / identificação *</label><input id="extension-label" maxlength="80" required placeholder="Ex.: Operação 24h"></div><div class="fgroup"><label>Ramal *</label><input id="extension-number" maxlength="30" required placeholder="Ex.: 204"></div></div><button class="btn btn-primary">Cadastrar ramal</button></form></section>':'')+'<section class="section-card"><div class="section-title"><h2>Ramais cadastrados</h2><p class="hint-text" id="extension-result-summary"></p></div><div id="extension-cards" class="extension-card-grid"></div></section>';
    ...
    function draw(){var query=String(document.getElementById('extension-search').value||'').trim().toLocaleLowerCase('pt-BR'),rows=(db.baseExtensions||[]).map(function(extension){var base=db.operationalBases.find(function(item){return item.id===extension.base_id;}),transporters=transporterNames(extension.base_id);return {extension:extension,base:base,transporters:transporters,haystack:[base&&base.name,base&&base.code,extension.label,extension.extension_number].concat(transporters).join(' ').toLocaleLowerCase('pt-BR')};}).filter(function(row){return row.base&&(!query||row.haystack.indexOf(query)!==-1);});document.getElementById('extension-result-summary').textContent=rows.length+' ramal(is) encontrado(s).';document.getElementById('extension-cards').innerHTML=rows.map(function(row){return '<article class="extension-card"><div class="extension-card-top"><span class="extension-base-code">'+escapeHtml(row.base.code)+'</span>'+(isAdmin?'<button class="extension-remove" data-extension-remove="'+row.extension.id+'" aria-label="Desativar ramal">×</button>':'')+'</div><h3>'+escapeHtml(row.base.name)+'</h3><p class="extension-carriers">'+escapeHtml(row.transporters.join(' · ')||'Sem transportadora vinculada')+'</p><div class="extension-number"><span>'+escapeHtml(row.extension.label)+'</span><strong>'+escapeHtml(row.extension.extension_number)+'</strong></div></article>';}).join('')||'<div class="empty-state">Nenhum ramal corresponde à pesquisa.</div>';root.querySelectorAll('[data-extension-remove]').forEach(function(button){button.onclick=async function(){if(!confirm('Desativar este ramal?'))return;var result=await supabaseClient.from('base_extensions').update({active:false,updated_at:new Date().toISOString()}).eq('id',button.dataset.extensionRemove);if(result.error){toast(result.error.message,'error');return;}await loadCloudData();draw();toast('Ramal desativado.','success');};});}
    document.getElementById('extension-search').oninput=draw;
    var form=document.getElementById('extension-form');if(form)form.onsubmit=async function(event){event.preventDefault();if(!supabaseClient||!db.supabaseConnected){toast('Conecte o SmartRisk ao Supabase.','error');return;}var result=await supabaseClient.from('base_extensions').insert({base_id:document.getElementById('extension-base').value,label:document.getElementById('extension-label').value.trim(),extension_number:document.getElementById('extension-number').value.trim(),created_by:user.id});if(result.error){toast(result.error.message,'error');return;}form.reset();await loadCloudData();draw();toast('Ramal cadastrado.','success');};
    draw();
  }
  ...
  function supportTicketCard(ticket,management){var requester=db.users.find(function(user){return user.id===ticket.requester_id;}),assignee=db.users.find(function(user){return user.id===ticket.assigned_to;});return '<article class="support-ticket-card" data-ticket-id="'+ticket.id+'"><div class="support-ticket-head"><div><span class="ticket-number">'+escapeHtml(ticket.ticket_number)+'</span><span class="ticket-priority priority-'+String(ticket.priority||'NORMAL').toLowerCase()+'">'+escapeHtml(supportPriorityLabel(ticket.priority))+'</span></div><span class="ticket-status status-'+String(ticket.status).toLowerCase()+'">'+escapeHtml(supportStatusLabel(ticket.status))+'</span></div><h3>'+escapeHtml(ticket.subject)+'</h3><p>'+escapeHtml(ticket.description)+'</p><dl><div><dt>Categoria</dt><dd>'+escapeHtml(ticket.category)+'</dd></div><div><dt>Solicitante</dt><dd>'+escapeHtml(requester?requester.nome:'Usuário')+'</dd></div><div><dt>Aberto em</dt><dd>'+new Date(ticket.created_at).toLocaleString('pt-BR')+'</dd></div>'+(management?'<div><dt>Responsável</dt><dd>'+escapeHtml(assignee?assignee.nome:'Não atribuído')+'</dd></div>':'')+'</dl>'+(ticket.response_text?'<div class="ticket-response"><b>Retorno da equipe</b><p>'+escapeHtml(ticket.response_text)+'</p></div>':'')+(management?'<button class="btn btn-primary btn-sm" data-ticket-manage="'+ticket.id+'">Tratar chamado</button>':'')+'</article>';}

  function renderSupportRequest(){
    var root=document.getElementById('support-request-root'),user=currentUser();if(!root)return;
    root.innerHTML='<div class="support-layout"><section class="section-card support-form-card"><div class="section-title"><span class="eyebrow">CENTRAL DE SUPORTE</span><h2>Como podemos ajudar?</h2><p class="hint-text">Descreva sua solicitação para que a equipe responsável possa analisar.</p></div><form id="support-request-form"><div class="form-grid two"><div class="fgroup"><label>Tipo de solicitação *</label><select id="support-category" required><option value="">Selecione</option><option>Criação de novo usuário</option><option>Dúvida sobre o sistema</option><option>Problema técnico</option><option>Solicitação de acesso</option><option>Correção de cadastro</option><option>Outro tipo de solicitação</option></select></div><div class="fgroup"><label>Prioridade *</label><select id="support-priority"><option value="LOW">Baixa</option><option value="NORMAL" selected>Normal</option><option value="HIGH">Alta</option><option value="URGENT">Urgente</option></select></div><div class="fgroup span-all"><label>Assunto *</label><input id="support-subject" required maxlength="160" placeholder="Resuma o que você precisa"></div><div class="fgroup span-all"><label>Descrição *</label><textarea id="support-description" required minlength="10" maxlength="4000" rows="6" placeholder="Explique a solicitação e inclua as informações necessárias para o atendimento."></textarea></div></div><button class="btn btn-primary" id="support-submit">Abrir chamado</button></form></section><section class="section-card support-my-tickets"><div class="section-title"><h2>Meus chamados</h2><p class="hint-text">Acompanhe o status e o retorno da equipe.</p></div><div id="support-my-list" class="support-ticket-list"></div></section></div>';
    function draw(){var rows=(db.supportTickets||[]).filter(function(ticket){return ticket.requester_id===user.id;});document.getElementById('support-my-list').innerHTML=rows.map(function(ticket){return supportTicketCard(ticket,false);}).join('')||'<div class="empty-state">Você ainda não abriu chamados.</div>';}
    document.getElementById('support-request-form').onsubmit=async function(event){event.preventDefault();if(!supabaseClient||!db.supabaseConnected){toast('Conecte o SmartRisk ao Supabase.','error');return;}var button=document.getElementById('support-submit');button.disabled=true;button.textContent='Enviando...';try{var result=await supabaseClient.rpc('create_support_ticket',{ticket_category:document.getElementById('support-category').value,ticket_subject:document.getElementById('support-subject').value.trim(),ticket_description:document.getElementById('support-description').value.trim(),ticket_priority:document.getElementById('support-priority').value});if(result.error)throw result.error;event.target.reset();await loadCloudData();draw();toast('Chamado aberto com sucesso.','success');refreshNotifBadge();}catch(error){console.error(error);toast(error.message||'Não foi possível abrir o chamado.','error');}finally{button.disabled=false;button.textContent='Abrir chamado';}};
    draw();
  }

  function renderSupportManagement(){
    var root=document.getElementById('support-management-root'),user=currentUser();if(!root)return;
    root.innerHTML='<section class="support-management-head"><div><span class="eyebrow">ATENDIMENTO INTERNO</span><h2>Fila de chamados</h2><p>Receba, atribua e responda às solicitações dos usuários.</p></div><div class="support-metrics"><div><strong id="support-open-count">0</strong><span>Em aberto</span></div><div><strong id="support-progress-count">0</strong><span>Em atendimento</span></div></div></section><section class="section-card"><div class="support-filters"><input id="support-search" type="search" placeholder="Pesquisar número, solicitante ou assunto"><select id="support-status-filter"><option value="">Todos os status</option><option value="OPEN">Aberto</option><option value="IN_PROGRESS">Em atendimento</option><option value="WAITING_USER">Aguardando usuário</option><option value="RESOLVED">Resolvido</option><option value="CLOSED">Encerrado</option></select></div><div id="support-management-list" class="support-ticket-list"></div></section><div class="modal-overlay" id="support-manage-modal"><div class="modal-card support-manage-card"><div class="modal-head"><div><span class="eyebrow">TRATAMENTO DO CHAMADO</span><h2 id="support-manage-title"></h2></div><button type="button" id="support-manage-close" aria-label="Fechar">×</button></div><form id="support-manage-form"><input type="hidden" id="support-manage-id"><div class="form-grid two"><div class="fgroup"><label>Status *</label><select id="support-manage-status"><option value="IN_PROGRESS">Em atendimento</option><option value="WAITING_USER">Aguardando usuário</option><option value="RESOLVED">Resolvido</option><option value="CLOSED">Encerrado</option></select></div><div class="fgroup"><label>Responsável</label><select id="support-manage-assignee"><option value="">Manter sem responsável</option>'+db.users.filter(function(candidate){return userPerms(candidate).chamados;}).map(function(candidate){return '<option value="'+candidate.id+'">'+escapeHtml(candidate.nome)+'</option>';}).join('')+'</select></div><div class="fgroup span-all"><label>Retorno ao solicitante</label><textarea id="support-manage-response" rows="5" maxlength="4000" placeholder="Registre a orientação ou solução aplicada."></textarea></div></div><div class="actions-row"><button type="button" class="btn btn-secondary" id="support-manage-cancel">Cancelar</button><button class="btn btn-primary" id="support-manage-submit">Salvar tratamento</button></div></form></div></div>';
    function draw(){var query=document.getElementById('support-search').value.trim().toLocaleLowerCase('pt-BR'),status=document.getElementById('support-status-filter').value,rows=(db.supportTickets||[]).filter(function(ticket){var requester=db.users.find(function(item){return item.id===ticket.requester_id;}),text=[ticket.ticket_number,ticket.subject,ticket.description,requester&&requester.nome].join(' ').toLocaleLowerCase('pt-BR');return (!query||text.indexOf(query)!==-1)&&(!status||ticket.status===status);});document.getElementById('support-open-count').textContent=(db.supportTickets||[]).filter(function(ticket){return ticket.status==='OPEN';}).length;document.getElementById('support-progress-count').textContent=(db.supportTickets||[]).filter(function(ticket){return ['IN_PROGRESS','WAITING_USER'].indexOf(ticket.status)!==-1;}).length;document.getElementById('support-management-list').innerHTML=rows.map(function(ticket){return supportTicketCard(ticket,true);}).join('')||'<div class="empty-state">Nenhum chamado encontrado.</div>';root.querySelectorAll('[data-ticket-manage]').forEach(function(button){button.onclick=function(){var ticket=db.supportTickets.find(function(item){return item.id===button.dataset.ticketManage;});if(!ticket)return;document.getElementById('support-manage-id').value=ticket.id;document.getElementById('support-manage-title').textContent=ticket.ticket_number+' · '+ticket.subject;document.getElementById('support-manage-status').value=ticket.status==='OPEN'?'IN_PROGRESS':ticket.status;document.getElementById('support-manage-assignee').value=ticket.assigned_to||user.id;document.getElementById('support-manage-response').value=ticket.response_text||'';document.getElementById('support-manage-modal').classList.add('show');};});}
